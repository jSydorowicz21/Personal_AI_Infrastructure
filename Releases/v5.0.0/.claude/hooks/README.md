...
Set `PAI_HOOK_RUN_LOG=1` (or reuse `PAI_HOOK_DEBUG=1`) to record every hook child run in:

`$PAI_DATA_DIR/MEMORY/OBSERVABILITY/hook-runs.jsonl`

Each entry is JSONL with:
- `hook-run-start` and `hook-run-end` events
- per-run `runId` for correlation
- target index (`1/3:hook.ts`) and adapter PID
- status, duration, timeout, and stderr preview

Quick check:

```powershell
Get-Content "$env:PAI_DATA_DIR\MEMORY\OBSERVABILITY\hook-runs.jsonl" -Tail 20
```
...
### Hooks Fail Immediately with `exit 1`, No Output, No Logs (Codex / Windows)

**Heuristic: when hooks fail instantly with no output and no logging is even
possible — not in `hook-runs.jsonl`, not from an unconditional beacon added to
the adapter, nothing — the hook script is NOT the problem. The command never
ran. That is almost always a bad command format: the shell can't parse the
`command` string, so the host process exits 1 before the interpreter starts.**

The canonical case: Codex executes hook commands through **PowerShell**, and
PowerShell will not run a line that begins with a quoted string — it parses
`"C:\...\bun.exe" "...\adapter.ts" ...` as an expression and raises a
`ParserError`. The fix is the PowerShell call operator `&`:

```
# BROKEN — PowerShell ParserError, hook reports `exited with code 1`
"C:\...\bun.exe" "C:\...\FrameworkHookAdapter.ts" --framework "codex" ...
# CORRECT
& "C:\...\bun.exe" "C:\...\FrameworkHookAdapter.ts" --framework "codex" ...
```

Generated by `PAI/PAI-Install/engine/config-gen.ts` → `hookCommandWindowsDirect`
(which prepends `&`), written into `hooks.json`. POSIX commands must NOT use `&`
(bash backgrounds the process) — only the Windows/PowerShell form needs it.

Fast triage:
1. Reproduce the exact `command`/`commandWindows` string in the host shell
   (PowerShell on Codex/Windows). A `ParserError` is the smoking gun.
2. Don't chase the hook script or trust ledger first — "trusted" + "exit 1" +
   zero output means the spawn itself failed, not the hook.
