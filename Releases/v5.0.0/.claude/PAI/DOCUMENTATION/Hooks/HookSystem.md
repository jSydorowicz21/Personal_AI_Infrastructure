...
- **History Capture** - Automatic work/learning documentation to `$PAI_DATA_DIR/MEMORY/`
...
**Session Mapping:** `$PAI_DATA_DIR/MEMORY/STATE/agent-sessions.json`
```json
{
...
**Check:**
1. Does `$PAI_DATA_DIR/MEMORY/` directory exist?
2. Does current-work file exist? Check `~/.claude/`
3. Is hook actually running? Check `$PAI_DATA_DIR/MEMORY/RAW/` for events
4. File permissions? `ls -la $PAI_DATA_DIR/MEMORY/WORK/`
...
ls -lt $PAI_DATA_DIR/MEMORY/WORK/ | head -10
ls -lt $PAI_DATA_DIR/MEMORY/LEARNING/$(date +%Y-%m)/ | head -10

# Check raw events
tail $PAI_DATA_DIR/MEMORY/RAW/$(date +%Y-%m)/$(date +%Y-%m-%d)_all-events.jsonl
```
...
**Check:**
1. Is `$PAI_DATA_DIR/MEMORY/STATE/agent-sessions.json` writable?
...
cat $PAI_DATA_DIR/MEMORY/STATE/agent-sessions.json | jq .
...
$PAI_DATA_DIR/MEMORY/WORK/               Work tracking
$PAI_DATA_DIR/MEMORY/LEARNING/           Learning captures
$PAI_DATA_DIR/MEMORY/STATE/              Runtime state
$PAI_DATA_DIR/MEMORY/STATE/events.jsonl  Unified event log (append-only)
$PAI_DATA_DIR/MEMORY/OBSERVABILITY/      Tool failures, agent spawns, config changes

INFERENCE TOOL (for hooks needing AI):
Path: $PAI_DIR/TOOLS/Inference.ts
Import: import { inference } from '$PAI_DIR/TOOLS/Inference'
...
// appendEvent() writes to $PAI_DATA_DIR/MEMORY/STATE/events.jsonl
...
tail -f $PAI_DATA_DIR/MEMORY/STATE/events.jsonl | jq

# Filter by type
tail -f $PAI_DATA_DIR/MEMORY/STATE/events.jsonl | jq 'select(.type | startswith("algorithm."))'

# Programmatic (Node/Bun fs.watch)
import { watch } from 'fs';
const eventsPath = `${process.env.PAI_DATA_DIR}/MEMORY/STATE/events.jsonl`;
