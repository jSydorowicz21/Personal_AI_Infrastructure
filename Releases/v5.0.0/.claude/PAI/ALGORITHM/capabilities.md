...
| FeedbackMemoryConsult | PLAN | **First step of PLAN at Extended+.** Before committing to approach, grep shared PAI feedback/learning memory by task keywords. Prevents repeating mistakes already documented and works across Claude, Codex, and OpenCode because the source is `PAI_DATA_DIR/MEMORY`, not harness-native project memory. | `Bash('rg -l "KEYWORDS" "$PAI_DATA_DIR/MEMORY/FEEDBACK" "$PAI_DATA_DIR/MEMORY/LEARNING" 2>/dev/null')` | E2+ |
