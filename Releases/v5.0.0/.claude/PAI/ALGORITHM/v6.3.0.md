...
**Mode and tier are decided by a standard-tier classifier at UserPromptSubmit, not by the executor.** `hooks/PromptProcessing.hook.ts` runs on every top-level prompt, calls the active framework through `Inference.ts`, and writes a single line into additionalContext:
...
3. For task ISAs the skill creates `$PAI_DATA_DIR/MEMORY/WORK/{slug}/`; for project ISAs the skill reads existing `<project>/ISA.md` if present, or seeds it via the Seed workflow
...
```bash
rg -i "TOPIC" $PAI_DATA_DIR/MEMORY/KNOWLEDGE/ --type md -l
```
...
2. For task work: read ISA from `$PAI_DATA_DIR/MEMORY/WORK/`
3. `$PAI_DATA_DIR/MEMORY/STATE/work.json` has the session registry
