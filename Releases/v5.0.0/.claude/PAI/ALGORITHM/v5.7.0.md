...
1. `mkdir -p $PAI_DATA_DIR/MEMORY/WORK/{slug}/` (slug: `YYYYMMDD-HHMMSS_kebab-task-description`)
...
You do not need to do anything to use this — write `[x]` honestly per Inline Verification, and the checkpoint trail forms itself. The trail enables clean rollback to any prior ISC state via `bun $PAI_DIR/TOOLS/Checkpoint.ts {list|show|rollback} <slug> [<isc-id>]`. Rollback is preview-only — it prints the suggested `git reset --hard <sha>` per repo and exits. {{PRINCIPAL_NAME}} runs the reset himself if he wants the rollback (per `feedback_no_worktree_isolation_without_consent`).
...
Or programmatically:
```typescript
import { advisor } from "$PAI_DIR/TOOLS/Inference";
...
```bash
rg -l "TOPIC|KEYWORD" $PAI_DATA_DIR/MEMORY/KNOWLEDGE/ --type md
```
...
1. Read ISA from `$PAI_DATA_DIR/MEMORY/WORK/` — full state
