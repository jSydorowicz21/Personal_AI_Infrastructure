...
```bash
bun $PAI_DIR/TOOLS/DAInterview.ts                  # Quick mode
bun $PAI_DIR/TOOLS/DAInterview.ts --depth standard # More detail
bun $PAI_DIR/TOOLS/DAInterview.ts --depth deep     # Every phase
```
...
```bash
bun $PAI_DIR/TOOLS/DAInterview.ts --update            # Update primary DA
bun $PAI_DIR/TOOLS/DAInterview.ts --update --da kai   # Update specific DA
```
